...
def assemble(server_exe: str, client_dir: str) -> str:
...
def stage_version(version: str) -> str:
    """Bake the version into the build so the updater knows what it is running.

    Generated rather than committed, and gitignored alongside the protocol copy: the tag
    is the source of truth, and a version tracked in two places drifts.
    """
    dst = os.path.join(ROOT, "client", "buildinfo.py")
    with open(dst, "w", encoding="utf-8") as f:
        f.write('"""Generated by build_dist.py. Do not edit."""\n\n')
        f.write(f'VERSION = "{version}"\n')
    return dst


def publish(version: str, zip_path: str) -> None:
    """Upload the build as a GitHub release, which is what the updater reads.

    Shells out to the GitHub CLI rather than talking to the API directly so that
    authentication is `gh auth login` and not a token this script has to store.
    """
    print(f"[5/5] publishing release {version}")
    if not shutil.which("gh"):
        raise SystemExit(
            "gh (GitHub CLI) not found. Install it with:\n"
            "    winget install GitHub.cli\n"
            "then sign in once with:\n"
            "    gh auth login"
        )

    subprocess.run(
        ["gh", "release", "create", version, zip_path,
         "--title", f"Asteroid Salvage {version}",
         "--notes", f"Automatic update build {version}."],
        check=True,
    )
    print(f"      published {version} — clients will pick it up on their next launch")


def next_version() -> str:
    """The next patch version after the newest release, or v0.1.0 if there are none."""
    if not shutil.which("gh"):
        return "v0.1.0"
    try:
        out = subprocess.run(
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Package Asteroid Salvage for sharing.")
    ap.add_argument(
        "--release",
        action="store_true",
        help="publish the build as a GitHub release, so shared copies auto-update",
    )
    ap.add_argument(
        "--version",
        default=None,
        help="version tag to stamp and publish (default: bump the latest release)",
    )
    args = ap.parse_args()

    version = args.version or (next_version() if args.release else "v0.0.0-dev")

    server = build_server()
    stage_protocol()
    stage_version(version)
    client = build_client()
    zip_path = assemble(server, client)
